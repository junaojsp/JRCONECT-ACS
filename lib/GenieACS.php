...
class GenieACS {
    ...
    public function parseDeviceData($device) {
        ...
        // Wi-Fi interfaces and cumulative traffic counters for overview TX/RX rates.
        // Read-only: values come from the GenieACS cache and no CPE task is triggered here.
        $wifiInterfaces = [];

        // TR-098 WLANConfiguration instances.
        for ($i = 1; $i <= 8; $i++) {
            $base = "InternetGatewayDevice.LANDevice.1.WLANConfiguration.{$i}";
            $ssid = $getParam("{$base}.SSID");
            $enable = $getParam("{$base}.Enable");
            $status = $getParam("{$base}.Status");
            $channel = $getParam("{$base}.Channel");
            $standard = $getParam("{$base}.Standard");
            $band = $getParam("{$base}.OperatingFrequencyBand") ??
                    $getParam("{$base}.X_FH_OperatingFrequencyBand") ??
                    $getParam("{$base}.X_HW_FrequencyBand");

            if ($ssid === null && $enable === null && $status === null && $channel === null && $standard === null) {
                continue;
            }

            if ($band === null && is_string($standard)) {
                $s = strtolower($standard);
                if (strpos($s, 'ac') !== false || strpos($s, 'a') !== false) $band = '5';
                elseif (strpos($s, 'b') !== false || strpos($s, 'g') !== false) $band = '2.4';
            }

            $wifiInterfaces[] = [
                'id' => $base,
                'label' => $band ? "Wi-Fi {$band} GHz" : "Wi-Fi {$i}",
                'ssid' => $ssid ?? 'N/A',
                'enabled' => $enable,
                ...
                'band' => $band,
                'clients' => $getParam("{$base}.TotalAssociations") ??
                             $getParam("{$base}.AssociatedDeviceNumberOfEntries") ?? null,
                'bytes_received' => $getParam("{$base}.Stats.BytesReceived"),
                'bytes_sent' => $getParam("{$base}.Stats.BytesSent"),
                'packets_received' => $getParam("{$base}.Stats.PacketsReceived"),
                'packets_sent' => $getParam("{$base}.Stats.PacketsSent"),
            ];
        }

        // TR-181 SSID instances. Stats normally live under Device.WiFi.SSID.{i}.Stats.
        if (empty($wifiInterfaces)) {
            for ($i = 1; $i <= 16; $i++) {
                $base = "Device.WiFi.SSID.{$i}";
                $ssid = $getParam("{$base}.SSID");
                $enable = $getParam("{$base}.Enable");
                $status = $getParam("{$base}.Status");
                $lowerLayers = $getParam("{$base}.LowerLayers");

                if ($ssid === null && $enable === null && $status === null && $lowerLayers === null) continue;

                $radioBase = null;
                if (is_string($lowerLayers) && preg_match('/Device\\.WiFi\\.Radio\\.\\d+/', $lowerLayers, $m)) {
                    $radioBase = $m[0];
                }
                $band = $radioBase ? ($getParam("{$radioBase}.OperatingFrequencyBand") ?? $getParam("{$radioBase}.SupportedFrequencyBands")) : null;
                $standard = $radioBase ? $getParam("{$radioBase}.OperatingStandards") : null;
                $channel = $radioBase ? $getParam("{$radioBase}.Channel") : null;

                $wifiInterfaces[] = [
                    'id' => $base,
                    'label' => $band ? "Wi-Fi {$band}" : "Wi-Fi {$i}",
                    'ssid' => $ssid ?? 'N/A',
                    'enabled' => $enable,
                    'status' => $status ?? 'Unknown',
                    'channel' => $channel ?? 'N/A',
        ...
        $data['wifi_interfaces'] = $wifiInterfaces;
